  ...
  module Union
    ...
    class Service
      ...
      def initialize *args, &block
        @options = Union::Options.default.dup.merge(args.extract_options!)
      ...
      def prepare_params params
        @params = Union::Params.new params
      ...
      def invoke method, http_method, *args
        ...
        puts [Union.gateways[:site], @params.to_query].join("?")
        begin
          raw_response = case http_method
          when :get then RestClient.get [Union.gateways[:site], @params.to_query].join("?")
          when :post then RestClient.post Union.gateways[:site], @params
          end
          @response = case @params.format.to_s.downcase
          when 'json'
            Union::Response.from_json(raw_response)
          when 'xml'
            Union::Response.from_xml(raw_response)
          else
            raw_response
          end
        rescue Exception => e
